...
def respawn_player():
    global velocity, currentztelpos, camera_locked, rot_locked, playlock, paused, camera_loc, gravity, cameraorb_pitch, cameraorb_yaw
    player.position = Vec3(0, 30, 0)
    velocity = 0
    player.z = zTelPos[2][2]
    currentztelpos = 2
    ...
    # Calculate initial camera position using orbit system
    cameraorb_yaw = 0
    cameraorb_pitch = 0
    # Calculate initial camera position for isometric view
    base_distance = 35
    base_height = 25
    base_pitch = -25  # Looking down
    
    yaw_rad = math.radians(225)  # Side angle
    pitch_rad = math.radians(base_pitch)
    
    horizontal_distance = base_distance * math.cos(pitch_rad)
    x_offset = horizontal_distance * math.cos(yaw_rad)
    z_offset = horizontal_distance * math.sin(yaw_rad)
    y_offset = base_height + (base_distance * math.sin(pitch_rad))
    
    camera.position = player.position + Vec3(x_offset, y_offset, z_offset)
    camera_loc = camera.position
...
def game_logic_step(dt):
    global velocity, is_grounded, currentztelpos, camera_loc, camera_locked, rot_locked, Sensitive, playlock, menu_music_playing, buttoncontrols, gravswapping, existing_gravswap, cameraorb_yaw, cameraorb_pitch
    ...
    # Testing camera movement logic for a new system
    if gravity != abs(gravity):
        base_distance = 35      
        base_height = 25        
        base_pitch = -25        
    ...
    if not camera_locked:
        if mouse.left:
            ...
            cameraorb_yaw -= mouse.velocity[0] * Sensitive * 10000 * dt
            cameraorb_pitch += mouse.velocity[1] * Sensitive * 10000 * dt
            
            # Normalize yaw to keep it within reasonable bounds
            cameraorb_yaw = cameraorb_yaw % 360
            if cameraorb_yaw > 180:
                cameraorb_yaw -= 360
            
            # Clamp pitch to keep level relatively flat
            cameraorb_pitch = max(-15, min(15, cameraorb_pitch))
            
            # Calculate camera position
            total_pitch = base_pitch + cameraorb_pitch
            yaw_rad = math.radians(cameraorb_yaw + 225)
            ...
            target_position = player.position + Vec3(x_offset, y_offset, z_offset)
            camera_loc = lerp(camera_loc, target_position, dt * 8)
            camera.position = camera_loc
            
        else:
            # Return to default isometric position with proper angle normalization
            target_yaw = 0
            target_pitch = 0
            
            # Find the shortest path to target yaw
            yaw_diff = target_yaw - cameraorb_yaw
            if yaw_diff > 180:
                yaw_diff -= 360
            elif yaw_diff < -180:
                yaw_diff += 360
            ...
            cameraorb_yaw += yaw_diff * return_speed * dt
            cameraorb_pitch = lerp(cameraorb_pitch, target_pitch, dt * return_speed)
            
            # Normalize yaw to prevent accumulation
            cameraorb_yaw = cameraorb_yaw % 360
            if cameraorb_yaw > 180:
                cameraorb_yaw -= 360
            ...
            return_location = player.position + Vec3(x_offset, y_offset, z_offset)
            camera_loc = lerp(camera_loc, return_location, dt * return_speed)
            camera.position = camera_loc
